...
const getPage = async (url2) => {
    ...
    const url1 = 'http://www.stats.gov.cn/tjsj/tjbz/tjyqhdmhcxhfdm/2019/';

    let response = '';
    try{
        response = (await superagent
            .get(url1 + url2).retry(20, function (error, response) {
                if (error || response.status === 302 || response.status === 502) {
                    console.log("即将retry " + url1 + url2);
...
const output = data => {
    ...
    console.log('查询完毕，即将写入文件data.json');
    fs.writeFileSync(
        'data.json',
        dataStr,
        function(err){
